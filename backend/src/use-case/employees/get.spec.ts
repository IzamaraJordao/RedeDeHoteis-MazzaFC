...
it ('should return an employee', async () => {
    ...
    const result = await sut.execute({params: {id: employee.id}, body: undefined, query: undefined});
    expect(result).toBe(employee);
});
it ('should throw if employee not found', async () => {
    const {sut} = makeSut();
   expect(sut.execute({params: {id: 'invalid_id'}, body: undefined, query: undefined})).rejects.toThrow();
});
